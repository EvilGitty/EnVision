...
class ImageColormap
{
	public:
		...
		cv::Mat image_setBrightnessContrast(cv::Mat Img, int colorId, bool saturation);

		// convert to negative
		cv::Mat image_setInversion(cv::Mat img);

		//set grayscale/ opencv ImageColormap
		cv::Mat image_setChannels(cv::Mat Img, int colorID);
