...
//
void SpecIL_Nuance::setImageProperties(const char* id, int width, int height, int depth, t_SpecHead::t_interleave il, t_SpecHead::t_data_type dt)
{
	//Err
...
int SpecIL_Nuance::getPoint(void* point, int x, int y, int z)
{
	...
	this->ErrorMsgLog("In SpecIL_Nuance::getPoint():");
	return -1;
};
//
		...
		if (x < 0 && y < 0 && z < this->SpecHeader.imgsize[1])
		{
			...
			if (this->OutputProfile == 1)
			{
				if (this->SpecHeader.interleave == t_SpecHead::IL_BSQ)
				{
					...
					//H					this->getPlane_Band(plane, y, x, z);	// Felix: PASST  DAS SO????
